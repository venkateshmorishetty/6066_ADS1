...
class BinarySearchSymbolTable {
	 ...
	 */
	public BinarySearchSymbolTable(int len) {
		keys = new String[len];
		values = new int[len];
	}
	/**
	...
	public int rank(String k) {
		...
		while(low <= high) {
			int mid = (low + high) / 2;
			if(keys[mid].compareTo(k) < 0) low = mid + 1;
			else if(keys[mid].compareTo(k) > 0) high = mid - 1;
			else return mid;
		}
		return low;
	}
...
class Solution {
	...
	public static void main(String[] args) {
		...
		BinarySearchSymbolTable s = new BinarySearchSymbolTable(keys.length);
